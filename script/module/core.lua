...
function M.newParse()
	...
	function XmlParser:ParseXmlText(xmlText)    
	  ...
	  while true do      
		...
		if (cdata_start) then
		  abs = i+cdata_start2       
		  log.info("Found CDATA start tag", abs)        
		  cdata_end,cdata_end2 = xmlText:find("%]%]>",i)
		  log.info("Found CDATA end tag", cdata_end2)      
		  i = cdata_end2+1
		else 
		  i = j+1
		end
		...
		elseif c == "" then   -- start tag
		  ...
		  log.info("openTag ="..top.name);
		else  -- end tag
		  local toclose = table.remove(stack)  -- remove top
		  log.info("closeTag="..toclose.name);
		  top = stack[#stack]
		  if #stack < 1 then
